...
public class PhotoGenerator : MonoBehaviour
{
    ...
    private Renderer _shootPhotoRenderer;
    ...
    public GameObject GeneratePhotoObject(ARLogUnit arLog, Texture2D photoTexture, bool isWorldPosition)
    {
    ...
    public void ApplyTexture(GameObject photoObject, Texture2D photoTexture)
    {
        if (photoObject.transform.childCount > 0)
        {
            ...
            _shootPhotoRenderer = child.GetComponent<Renderer>();
            if (_shootPhotoRenderer != null) _shootPhotoRenderer.material.mainTexture = photoTexture;
        }
    }
    public void UpdateTexture(Texture2D photoTexture)
    {
        if (_shootPhotoRenderer != null) _shootPhotoRenderer.material.mainTexture = photoTexture;
    }
}
